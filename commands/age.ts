...
export function age({ threshold = '1 year ago', top = 20 }: AgeOptions = {}): void {
  ...
  for (const { file, lastTouch, daysAgo } of display) {
    const color =
      daysAgo > 730 ? chalk.red : daysAgo > 450 ? chalk.yellow : chalk.white
  ...
  console.log()
  warn(
    `${results.length} stale files total. These may be dead code, abandoned features, or forgotten utilities.`,
  )
  info(
    `Consider removing, archiving, or adding a deprecation notice to the oldest ones.`,
  )
  console.log()
}
