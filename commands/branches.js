...
export function branches({ threshold = '3 months ago' } = {}) {
  header('STALE BRANCHES', `Remote branches with no activity since "${threshold}"`);
  ...
  for (const branch of remoteBranches) {
    ...
    if (d < cutoff) {
      ...
      stale.push({ branch: branch.replace('origin/', ''), date, daysAgo, author, subject });
    }
  }
  ...
  const table = makeTable(['Last Activity', 'Days Ago', 'Author', 'Branch'], [14, 10, 20, 24]);
  for (const { branch, date, daysAgo, author } of stale) {
    const color = daysAgo > 180 ? chalk.red : daysAgo > 90 ? chalk.yellow : chalk.white;
    const shortAuthor = author.length > 18 ? author.slice(0, 17) + '…' : author;
    const shortBranch = branch.length > 22 ? branch.slice(0, 21) + '…' : branch;
    table.push([color(date), color(String(daysAgo)), chalk.dim(shortAuthor), color(shortBranch)]);
  }
