...
export function fires({ since = '1 year ago' } = {}) {
  ...
  const raw = git(`log --oneline --since="${since}"`)
  ...
  if (!matches.length) {
    ...
    info(
      `Either things are stable, or commit messages don't use these keywords.`,
    )
    console.log()
    return
  }
  ...
  for (const [month, count] of monthEntries) {
    const bar =
      chalk.red('▓'.repeat(count)) +
      chalk.dim('░'.repeat(Math.max(0, maxVal - count)))
  ...
  console.log(
    chalk.bold(
      `  Total: ${chalk.red.bold(matches.length)} fire commits out of ${chalk.white(all.length)} (${((matches.length / all.length) * 100).toFixed(1)}%)`,
    ),
  )
  console.log()
  ...
  if (perMonth >= 2) {
    warn(
      `Averaging ${perMonth.toFixed(1)} fire commits/month — team may not trust the deploy process.`,
    )
  } else if (perMonth >= 1) {
    warn(
      `Some fire commits present. Worth checking test coverage and staging reliability.`,
    )
  } else {
