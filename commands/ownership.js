...
export function ownership({ top = 15 } = {}) {
  ...
  const table = makeTable(['Top Owner', 'Own%', 'Contributors', 'File'], [22, 7, 14, 25]);
  ...
  for (const file of topFiles) {
    ...
    const color = parseInt(pct) > 80 ? chalk.red : parseInt(pct) > 60 ? chalk.yellow : chalk.green;
    // Truncate author name to fit
    const shortAuthor = topAuthor.length > 18 ? topAuthor.slice(0, 16) + '..' : topAuthor;
    const shortFile = file.length > 23 ? '…' + file.slice(-22) : file;

    table.push([color(shortAuthor), color(`${pct}%`), chalk.white(String(contribs)), chalk.dim(shortFile)]);
  }
