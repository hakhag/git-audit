...
export function coupling({
  ...
  if (!sorted.length) {
    console.log(
      chalk.dim(`  No file pairs changed together ${minCount}+ times.\n`),
    )
    return
  }

  const table = makeTable(['Co-changes', 'File Pair'], [12, 100])
  for (const [pair, count] of sorted) {
    const color =
      count > 10 ? chalk.red.bold : count > 5 ? chalk.yellow : chalk.white
  ...
  console.log()
  info(
    `High coupling between files suggests implicit dependencies or missing abstractions.`,
  )
  console.log()
}
