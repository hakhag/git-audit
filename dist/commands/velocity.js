...
export function velocity() {
    ...
    for (const [month, count] of months) {
        ...
        if (pct > 0.75)
            ...
            barColor = chalk.red;
        console.log(`  ${chalk.dim(month)}  ${barColor(bar)}  ${chalk.white(label)}`);
    }
    if (months.length >= 3) {
        ...
        if (older.length) {
            ...
            console.log();
            if (change < -40) {
                warn(`Velocity down ${Math.abs(change).toFixed(0)}% vs 3 months prior — team losing momentum.`);
            }
            else if (change > 40) {
                info(`Velocity up ${change.toFixed(0)}% vs 3 months prior — accelerating.`);
            }
            else {
